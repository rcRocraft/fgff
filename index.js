...
client.once('ready', () => {
    ...
    client.user.setActivity(`${config.prefix}help │ KundoGen FREE`, { type: "WATCHING" }); // Set the bot's activity status
...
client.on('message', (message) => {
	...
	if (config.command.notfound_message === true && !client.commands.has(command)) {
        return message.channel.send(
            ...
            .setDescription(`Désolée, mais je n'ai pas trouver la commande \`${command}\`!`)
	...
	try {
		...
		if (config.command.error_message === true) {
            message.channel.send(
                ...
                .setColor(config.color.red)
                .setTitle('Erreur!')
                .setDescription(`une erreur c'est produite en utilisant la commande \`${command}\``)
                .addField('Error', `\`\`\`js\n${error}\n\`\`\``)
                .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                .setTimestamp()
            );
